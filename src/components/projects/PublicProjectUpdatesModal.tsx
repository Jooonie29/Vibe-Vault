...
export function PublicProjectUpdatesModal({ isOpen, onClose, token, projects }: PublicProjectUpdatesModalProps) {
  ...
  // Fetch all updates via public token
  ...
  // Note: This assumes getProfilesByUserIds is accessible publicly or we might miss avatar/names
          ...
          <div className="flex items-center gap-3">
            <div className="relative group">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2.5 bg-muted/50 border-transparent hover:bg-muted focus:bg-card border hover:border-border focus:border-violet-200 rounded-xl text-sm focus:outline-none focus:ring-4 focus:ring-violet-500/10 w-64 transition-all"
                value={filters.query}
            ...
            <div className="flex-1 flex min-w-0">
              {weekDays.map((day, dayIndex) => {
                ...
                return (
                  <div
                    key={dayKey}
                    className={`flex-1 min-w-0 border-r border-border/50 relative group ${dayIndex === 6 ? 'border-r-0' : ''} ${isTodayDate ? 'bg-violet-50/5 dark:bg-violet-900/5' : ''}`}
                  >
                    ...
                    <div className={`sticky top-0 z-20 h-16 border-b border-border flex flex-col items-center justify-center gap-1 shadow-sm transition-colors ${isTodayDate ? 'bg-violet-50/95 dark:bg-violet-900/20 backdrop-blur-sm' : 'bg-card/95 backdrop-blur-sm'}`}>
                      ...
                      <div className={`text-lg font-bold w-8 h-8 flex items-center justify-center rounded-full transition-all ${isTodayDate ? 'bg-violet-600 dark:bg-violet-500 text-white shadow-lg shadow-violet-500/30 dark:shadow-none scale-110' : 'text-foreground group-hover:bg-muted group-hover:scale-105'}`}>
                        {format(day, 'd')}
                      </div>
                    </div>
